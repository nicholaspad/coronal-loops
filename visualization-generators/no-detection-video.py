...
PRINTER = Recorder()
PRINTER.display_start_time("no-detection-video")
...
os.system("ffmpeg -y -f image2 -start_number 000 -framerate %s -i %s/resources/cutout-images/cut-%%3d.jpg -q:v 2 -vcodec mpeg4 -b:v 800k \"%s/videos/%s.mp4\"" % (FRAMERATE, MAIN_DIR, MAIN_DIR, NAME))

PRINTER.info_text("Done: video saved to 'videos/'")
RECORDER.display_end_time("no-detection-video")
