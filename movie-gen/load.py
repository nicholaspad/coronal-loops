...
class Load(object):
	...
	def processdata(self):
		...
		if clear == "y":
			...
			os.system("rm /Users/%s/Desktop/lmsal/movie-gen/src/*.jp2" % getpass.getuser())
			print ""
		else:
		...
		for i in trange(self.period, desc = "LOADING DATA", smoothing = 1):
			...
			with open(f) as file:
				os.rename(f, "/Users/%s/Desktop/lmsal/movie-gen/src/img_%03d.jp2" % (getpass.getuser(), ticker))

			ticker += 1
			self.time = self.time + timedelta(minutes = self.interval)

		os.system("ffmpeg -f image2 -start_number 000 -framerate %s -i /Users/%s/Desktop/lmsal/movie-gen/src/img_%%3d.jp2 -pix_fmt yuv420p -s 2048x2048 /Users/%s/Desktop/lmsal/movie-gen/%s.mp4" % (self.fps, getpass.getuser(), getpass.getuser(), str(self.time.date())))
